 ...
 * 
 * CREDITS:
 * Author: Miguel L�pez Ram�rez "Neowedge"
...
function NeoApplication(canvas)
{
	...
	var totalSoundsCount = 0;
	var loading = false;
	
	this.getLoadedImagesCount = function()	{	return loadedImagesCount;	}
	this.getLoadingSoundsCount = function()	{	return loadedSoundsCount;	}
	this.getTotalImagesCount = function()	{	return totalImagesCount;	}
	this.getTotalSoundsCount = function()	{	return totalSoundsCount;	}
	...
	{
		return new NeoBound(0, 0, context.canvas.width, context.canvas.height);
	}
	...
	{
		loader.addImage(id, url, tags, priority);
		totalImagesCount++;
	}
	...
	{
		loader.addSound(id, url, tags, priority);
		totalSoundsCount++;
	}
	 ...
	 * 
	 */
	this.startLoad(orderedTags)
	{
		if (!loading) //TODO: Test without verification (!loading)
		{
			loading = true;
			loader.addProgressListener(resourceLoaded);
			loader.addCompletionListener(loadCompleted)
			loader.start(orderedTags);
		}
	}
	...
	var resourceLoaded = function(e)
	{
		...
		{
			sounds[e.resource.id] = e.resource.sound;
			loadedSoundsCount++;
		}
	}
	 ...
	 * 
	 */
	this.playSound = function(sound, options)
	{
		sounds[sound].play(sound, options);
	}
 ...
 * @returns {NeoLevel}
 */
function NeoLevel(application, name)
{
	var _application = app;
	var _name = name;
	
	var _bound = new NeoBound(0, 0, _application.context.canvas.width, _application.context.canvas.height);
	 ...
	 * 
	 */
	this.tracePoint = function(x, y, relative)
	{
		...
		for (var i in _children)
		{
			...
			if (_children[i] instanceof NeoLayer)
				entities.join(_children[i].tracePoint(x, y, relative));
		}
		return entities;
	}
	 ...
	 * 
	 */
	this.startDrag = new function(entity, x, y)
	{
		_draggedEntity = entity;
		_dragXInset = entity.bound.x - x;
		_dragYInset = entity.bound.y - y;
	}
	...
	this.drag = function(x, y)
	{
		...
		{
			_draggedEntity.locate(x - _dragXInset, y - _dragYInset, false);
		}
	}
	...
	this.setTimeFrame = function(milliseconds)
	{
		...
		{
			clearInterval(timer);
			timer = setInterval(run, timeFrame);
		}
	}
	...
	this.draw = function()
	{
		...
		for (var i in _children) if (_children[i] != _draggedEntity)
			_children[i].draw(_application.context);
		
		//Drag
		if (_draggedEntity != null)
			_draggedEntity.draw(application.context);
	}
}
...
function NeoEntity()
{
	this.bound = new NeoBound(0, 0, 0, 0);
	this.rotation = 0;
		
	this.visible = true;
	var _level = null;
	var _parent = null;
	var _dragDropEnabled = false;
	...
	this.getAbsoluteBound = function()
	{
		...
		var absoluteWidth = Math.min(this.bound.width, refBound.width - Math.min(this.bound.x, refBound.width));
		var absoluteHeight = Math.min(this.bound.height, refBound.height - Math.min(this.bound.y, refBound.height));
	 ...
	 * 
	 */
	var setLevel = function(entity, removeParent)
	{
	...
	this.setDragDropEnabled = function(enabled)
	{
		_dragDropEnabled = enabled;
		if (_dragDropEnabled)
		{
			this.addEventListener("mousedown", onDragDown);
			this.addEventListener("touchstart", onDragDown);
			
			this.addEventListener("mousemove", onDragMove);
			this.addEventListener("touchmove", onDragMove;
			
			this.addEventListener("mouseup", onDragUp);
			this.addEventListener("touchend", onDragUp);
		}
		else
		{
			this.removeEventListener("mousedown", onDragDown);
			this.removeEventListener("touchstart", onDragDown);
			
			this.removeEventListener("mousemove", onDragMove);
			this.removeEventListener("touchmove", onDragMove;
			
			this.removeEventListener("mouseup", onDragUp);
			this.removeEventListener("touchend", onDragUp);
		}
	}
...
function NeoLayer()
{
	...
	this.tracePoint = function(x, y, relative)
	{
		...
		for (var i in _children)
		{
			...
			if (_children[i] instanceof NeoLayer)
				entities.concat(_children[i].tracePoint(x, y, relative));
		}
 ...
 * @param textBaseline
 */
function NeoLabel(text, color, fontFamily, fontSize, fontStyle, textAlign, textBaseline)
{
	this.text = text;
	this.color = color == null ? "auto" : color;
	this.fontFamily = fontFamily == null ? "auto" : fontFamily;
	this.fontSize = fontSize == null ? "auto" : fontSize;
	this.fontStyle = fontStyle == null ? "auto" : fontStyle;
	...
	this.draw = function(context)
	{
		if (this.visible)
		{
			...
			context.fillText(this.text, bound.x, bound.y, bound.width);
			context.restore();
		}
	}
}
 ...
 * @param y
 */
function NeoSprite(image, x, y)
{
	this.image = image;
	this.bound.x = x;
	this.bound.y = y;
	this.bound.width = image.width;
	this.bound.height = image.height;
	this.scaleX = 1;
	this.scaleY = 1;
	...
	this.draw = function(context)
	{
		if (this.visible)
		{
			...
			context.rotate(this.rotation);
			context.setScale(scaleX, scaleY);
			context.drawImage(this.image, bound.x, bound.y, bound.width, bound.height);
			context.restore();
		}
	}
}
...
function NeoAnimation(image, frameHeight, frameWidth, frameMap)
{
	NeoSprite.call(this, image);
	this.playAnimation(name)
	{
		
	}
}
NeoAnimation.prototype = new NeoSprite();
 ...
 * @param tileWidth
 */
function NeoTileset(image, tileHeight, tileWidth)
{
	NeoSprite.call(this, image);
	
	this.createImage = function(tilemap)
	{
		
	}
}
NeoTileset.prototype = new NeoSprite();
